...
**Tock** — logo files in `docs/branding/`, regenerated by
`scripts/generate_brand_assets.py`. The repo directory is
`analog_clock_prediction`.
...
1. **Class order is alphabetical over the UNDERSCORE labels** from
   ...
   names instead scores 66.6%, with every error a whole number of hours. The
   CSV's `class index` column is **identical** to that directory ordering and
   scores the same 66.6% (measured 2026-08-29 — there are two orderings, not
   three; the 65% previously quoted here does not reproduce). Fails *silently*.
   Always use `clockmodel.class_names()`.
...
4. **The resize filter changes the accuracy, and flips checkpoint rankings**
   (2026-08-29). 224→150 downsampling is unpinned in this repo. Released model:
   0.9938 bilinear / **0.9986 nearest**; `clock_model_rot54_s0`: **0.9979**
   bilinear / 0.9812 nearest. Each wins under the filter it was trained with —
   the released one came from `ImageDataGenerator` (default `nearest`),
   `clockmodel.make_dataset` uses `image_dataset_from_directory` (default
   `bilinear`). The released model's documented `11-10 → 7:55` failure mode is
   an artifact of this and disappears entirely under `nearest`. **Pin the
   filter before comparing checkpoints**, and treat the 0.9938-vs-0.9979 gap
   quoted in README.md as partly a preprocessing measurement.
5. **`train.py --backbone resnet50v2` is known-broken** (2026-08-27 ablation:
